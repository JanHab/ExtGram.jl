...
u_prim = []
for x in 1:Nloc
    push!(u_prim, moment_cons2prim(SVector{M+1}(Fmat[:, x]...), equations))
end
u_prim = hcat(u_prim...)
# u3 = Fmat[4, :]  # fourth moment
Q = u_prim[4, :]

plot(x, ρ[:, end], label="ρ", lw=2)
plot(x, ρ[:, end], label="ρ", lw=2, xlim=(-1, 0))
plot(x, v[:, end], label="v", lw=2)
plot(x, v[:, end], label="v", lw=2, xlim=(-1, 0))
plot(x, p[:, end], label="p", lw=2)
plot(x, p[:, end], label="p", lw=2, xlim=(-1, 0))
plot(x, Q, label="Q", lw=2)
plot(x, Q, label="Q", lw=2, xlim=(-1, 0))