...
for M in Moments:
    for chi_value in chi_vector:
        ...
        createsbatch(
            ...
            output_file=f"out/Riemann1D/GaugeInvestigation/slurm_output_M{M}_closure{closure}_Kn{Kn}_source{source}_T{T_end}_level{base_tree_level}_p{polydeg}_chi{chi_value}_rho_L{rho_L}_v_L{v_L}_theta_L{theta_L}_rho_R{rho_R}_v_R{v_R}_theta_R{theta_R}.out"
        )